...
program monteCarloDriver
  ...
  if(MasterProc) then ! Write a single output file. 
    ...
    !
    ! Write scattering order results for direct/diffuse separation
    !
    call writeScatteringOrderResults(mcIntegrator, './Output/MonteCarloDriver/', 'Diffuse_Fluxes.out')
    print *, "Wrote direct/diffuse separation results"
  end if
