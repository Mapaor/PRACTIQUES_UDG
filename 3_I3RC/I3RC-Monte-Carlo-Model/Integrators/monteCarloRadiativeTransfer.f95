...
module monteCarloRadiativeTransfer
  ...
  type integrator
    ...
    integer                           :: maxScatteringOrder = 10
    real, dimension(:, :, :), pointer :: fluxUpByOrder => null()
    real, dimension(:, :, :), pointer :: fluxDownByOrder => null()
  ...
  public :: new_Integrator, copy_Integrator, isReady_Integrator, finalize_Integrator, &
            specifyParameters, computeRadiativeTransfer, reportResults, writeScatteringOrderResults
contains
  !------------------------------------------------------------------------------------------
  ...
  function new_Integrator(atmosphere, status) result(new)
    ...
    if(stateIsFailure(status)) then
      ...
      ! Allocate scattering order arrays - allowing for up to 20 scattering orders
      new%maxScatteringOrder = 20
  ...
  subroutine computeRadiativeTransfer(thisIntegrator, randomNumbers, incomingPhotons, status)
    ...
    if(.not. isReady_integrator(thisIntegrator)) then
      ...
      ! Normalize scattering order fluxes by average number of photons per column
      do k = 0, thisIntegrator%maxScatteringOrder
        thisIntegrator%fluxUpByOrder(:, :, k)   = thisIntegrator%fluxUpByOrder(:, :, k)   / numPhotonsPerColumn(:, :)
        thisIntegrator%fluxDownByOrder(:, :, k) = thisIntegrator%fluxDownByOrder(:, :, k) / numPhotonsPerColumn(:, :)
      end do 
      !
    ...
    photonLoop: do
      ...
      scatteringLoop: do
        ...
        if(zPos >= zMax) then
          ...
          ! Track flux by scattering order - limit to max array size, put overflow in highest bin
          if(scatteringOrder <= thisIntegrator%maxScatteringOrder) then
            ...
            ! Photons with scattering order > maxScatteringOrder go into the highest bin
            thisIntegrator%fluxUpByOrder(xIndex, yIndex, thisIntegrator%maxScatteringOrder) = &
              thisIntegrator%fluxUpByOrder(xIndex, yIndex, thisIntegrator%maxScatteringOrder) + photonWeight
          end if
          cycle photonLoop
          ...
          ! Track flux by scattering order - limit to max array size, put overflow in highest bin
          if(scatteringOrder <= thisIntegrator%maxScatteringOrder) then
            thisIntegrator%fluxDownByOrder(xIndex, yIndex, scatteringOrder) = &
              thisIntegrator%fluxDownByOrder(xIndex, yIndex, scatteringOrder) + photonWeight
          else
            ! Photons with scattering order > maxScatteringOrder go into the highest bin
            thisIntegrator%fluxDownByOrder(xIndex, yIndex, thisIntegrator%maxScatteringOrder) = &
              thisIntegrator%fluxDownByOrder(xIndex, yIndex, thisIntegrator%maxScatteringOrder) + photonWeight
          end if
          !
  ...
  subroutine finalize_Integrator(thisIntegrator)
    ...
    ! Deallocate scattering order arrays
    if(associated(thisIntegrator%fluxUpByOrder))    deallocate(thisIntegrator%fluxUpByOrder)
    if(associated(thisIntegrator%fluxDownByOrder))  deallocate(thisIntegrator%fluxDownByOrder)
    
    ! Reset maxScatteringOrder after deallocation
    thisIntegrator%maxScatteringOrder = pristineI%maxScatteringOrder
  ...
  !------------------------------------------------------------------------------------------
  subroutine writeScatteringOrderResults(thisIntegrator, directoryName, filename)
    type(integrator), intent(in) :: thisIntegrator
    character(len=*), intent(in) :: directoryName, filename
    
    ! Local variables
    character(len=256) :: fullFileName
    integer :: i, j, k, fileUnit
    real :: topDnDirect, botDnDirect, topDnDiffuse, botDnDiffuse
    
    ! Construct full file path
    fullFileName = trim(directoryName) // "/" // trim(filename)
    
    ! Open file for writing
    fileUnit = 20
    open(unit=fileUnit, file=trim(fullFileName), status='replace')
    
    ! Write header
    write(fileUnit, '(A)') "# Direct and Diffuse Radiation Results"
    write(fileUnit, '(A)') "# TOPDN (direct), BOTDN (direct), TOPDN (diffuse), BOTDN (diffuse)"
    write(fileUnit, '(A)') "# Order 0 = Direct radiation, Order >= 1 = Diffuse radiation"
    
    ! Loop per cada punt de la malla
    do j = 1, size(thisIntegrator%fluxDown, 2)
      do i = 1, size(thisIntegrator%fluxDown, 1)
        ! Radiació directa (scattering order 0)
        if(0 <= thisIntegrator%maxScatteringOrder) then
          topDnDirect = thisIntegrator%fluxUpByOrder(i, j, 0)
          botDnDirect = thisIntegrator%fluxDownByOrder(i, j, 0)
        else
          topDnDirect = 0.0
          botDnDirect = 0.0
        end if
        
        ! Radiació difusa (scattering order >= 1)
        topDnDiffuse = 0.0
        botDnDiffuse = 0.0
        do k = 1, thisIntegrator%maxScatteringOrder
          topDnDiffuse = topDnDiffuse + thisIntegrator%fluxUpByOrder(i, j, k)
          botDnDiffuse = botDnDiffuse + thisIntegrator%fluxDownByOrder(i, j, k)
        end do
        
        ! Flux radiatiu per aquest punt de la malla
        write(fileUnit, '(4E15.6)') topDnDirect, botDnDirect, topDnDiffuse, botDnDiffuse
        
        ! Debugging
        write(fileUnit, '(A,2E15.6)') "# Total_flux_by_order: ", topDnDirect + topDnDiffuse, botDnDirect + botDnDiffuse
        write(fileUnit, '(A,2E15.6)') "# Original_total_flux: ", thisIntegrator%fluxUp(i,j), thisIntegrator%fluxDown(i,j)
      end do
    end do
    
    close(fileUnit)
    
    print *, "Direct/diffuse results written to: ", trim(fullFileName)
  end subroutine writeScatteringOrderResults
  !------------------------------------------------------------------------------------------
